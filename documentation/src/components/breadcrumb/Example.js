...
const Example = () => {
  ...
  const updateBreadcrumb = useCallback(breadCrumbitem => {
    const breadcrumbIndex = breadcrumbs.findIndex(item => item.url === breadCrumbitem.url);
    let updatedBreadcrumbs = breadcrumbs.slice();

    if (breadcrumbIndex !== -1)
      updatedBreadcrumbs.splice(breadcrumbIndex, 1);
    else
      updatedBreadcrumbs.push(breadCrumbitem);
